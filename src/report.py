...
def build_dashboard(runs: list[dict], generated_at: str, errors: list[dict] | None = None) -> str:
    ...
    lines = [
        ...
        f"| Median dispatch-to-PR detected | {f'{median_to_pr} minutes' if median_to_pr is not None else 'n/a'} |",
        f"| Median dispatch-to-green | {f'{median_to_green} minutes' if median_to_green is not None else 'n/a'} |",
    ...
    lines += [
        ...
        " Counts are shown with denominators: this is a four-run pilot, not a rate._",
    ]
    return "\n".join(lines)
