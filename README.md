...
## Results and evidence (pilot cohort)

Every claim below links to public evidence in the
[Superset fork](https://github.com/sb1992/superset). No PR was auto-merged; all
remain open for human review.

**Live dashboard:** [issue #1](https://github.com/sb1992/superset/issues/1) —
regenerated by the controller on every dispatch and reconciliation.

| Issue | Work | Devin PR | Outcome |
|---|---|---|---|
| [#2](https://github.com/sb1992/superset/issues/2) | Replace deprecated `datetime.utcnow()` in three unit-test files | [#4](https://github.com/sb1992/superset/pull/4) | Selected gates green |
| [#3](https://github.com/sb1992/superset/issues/3) | Diagnose a test skipped "works locally but fails on CI" | [#5](https://github.com/sb1992/superset/pull/5) | Selected gates green |
| [#7](https://github.com/sb1992/superset/issues/7) | Upgrade `cryptography` for PYSEC-2026-3552 | [#9](https://github.com/sb1992/superset/pull/9) | Selected gates green |
| [#8](https://github.com/sb1992/superset/issues/8) | Reassess the PyJWT subject-verification workaround | [#10](https://github.com/sb1992/superset/pull/10) | **Blocked — needs a human** |
| [#11](https://github.com/sb1992/superset/issues/11) | Independent replication of #3 | [#12](https://github.com/sb1992/superset/pull/12) | Selected gates green |

**How to read the outcomes.** Three remediations passed the controller's
selected Python gates (`unit-tests*` and `pre-commit`); one was blocked because
the session ended without the required structured output. Other checks on those
PRs (fork-infrastructure and upstream jobs such as dependency review, which needs
a dependency graph the fork does not enable) are displayed separately in each
status comment and do not determine the result.

**The hero remediation.** [Issue #3](https://github.com/sb1992/superset/issues/3)
proposed a hypothesis: a hard-coded database ID. Devin reproduced the failure,
disproved that hypothesis, and identified the real cause — `SyncPermissionsCommand`
raising `UserNotFoundInSessionError` inside `UpdateDatabaseCommand.run()`, rolling
back the update. [PR #5](https://github.com/sb1992/superset/pull/5) fixes the
isolation properly.

**Reproducibility.** [Issue #11](https://github.com/sb1992/superset/issues/11) is
an independent replication of that diagnosis, instructed to ignore existing
branches and PRs. It reached the same root cause and a near-identical fix
([PR #12](https://github.com/sb1992/superset/pull/12)) — and was dispatched
through the hosted label path
([Actions run](https://github.com/sb1992/superset/actions/runs/31147545542),
`event: issues`).

**Governance in practice.** [Issue #8](https://github.com/sb1992/superset/issues/8)
produced a PR whose selected gates were green, and the controller still refused
to mark it successful: the session stopped at its per-session usage limit before
providing structured output, so the evidence contract was unmet and the run
